...
namespace libreism::api::v1 {
    ...
    void TimeHttpController::getServerTimeJson(const drogon::HttpRequestPtr& req, Callback&& callback, std::string&& url) const {
        ...
        const auto serverTime = std::chrono::system_clock::from_time_t(std::mktime(&tm)) + (timeEnd - timeBegin);
        const int64_t serverUnixTimeMs = std::chrono::duration_cast<std::chrono::milliseconds>(serverTime.time_since_epoch()).count();
        const int64_t standardUnixTimeMs = std::chrono::duration_cast<std::chrono::milliseconds>(nistStandardTime.time_since_epoch()).count();
        const int64_t networkLatency = std::chrono::duration_cast<std::chrono::milliseconds>(timeEnd - timeBegin).count();
